...
function cargarEnv() {
  for (const nombre of [".env.local", ".env"]) {
    const envPath = resolve(process.cwd(), nombre);
...
async function main() {
  const seedPath = resolve(process.cwd(), "prisma/seed-data.json");
  if (!existsSync(seedPath)) {
    throw new Error(
      "Falta prisma/seed-data.json. Desde la raíz ejecutá: npm run catalogo:export:api"
    );
  }
